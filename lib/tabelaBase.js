...
module.exports = {
  ...
  getCamposNomeSelect(fields, fieldsConsulta, tableName) {
    let camposNome = [];
    let camposGroup = [];
    let groupBy = false;

    if (fieldsConsulta) {
      if (fieldsConsulta.tabela === undefined) {
    ...
    for (let campoConsulta of criterios) {
      ...
      switch (campoConsulta.operador) {
        case Operador.CONTEM:
          consulta.push(`${tableName}.${campoName.toUpperCase()}${collate} LIKE ? ${campoConsulta.comparador}`);
          break;
        case Operador.INICIA_COM:
          consulta.push(`${tableName}.${campoName.toUpperCase()}${collate} LIKE ? ${campoConsulta.comparador}`);
          break;
        case Operador.INTERVALO:
          consulta.push(`${tableName}.${campoName.toUpperCase()}${collate} BETWEEN ? AND ? ${campoConsulta.comparador}`);
          break;
        default:
          consulta.push(`${tableName}.${campoName.toUpperCase()}${collate} ${campoConsulta.operador} ? ${campoConsulta.comparador}`);
      }
  ...
  prepararOrderBy(tableNameDefault, orderBy, fields) {
    ...
    for (let campoOrder of orderBy) {
      ...
      if (campo.tableName) {
        order.push(campo.tableName.toUpperCase() + '.' + campo.name.toUpperCase() + collate);
      } else {
        order.push(campo.name.toUpperCase() + collate);
      }
    }
