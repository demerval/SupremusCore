...
class Tabela {
    ...
    async consultar(criterio, orderBy, fieldsConsulta, groupTable = true) {
        let sql = GeradorSql.sqlConsulta(this, fieldsConsulta);
        ...
        if (orderBy) {
            let strOrderBy = tabelaBase.prepararOrderBy(this.getNome(), orderBy, this._fields);
            if (strOrderBy) {
    ...
    async consultaPaginada(first, skip, criterio, orderBy, fieldsConsulta, groupTable = true) {
        try {
            let totalReg = 0;
            let sql = GeradorSql.sqlConsultaPaginada(this, fieldsConsulta);
