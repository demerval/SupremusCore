...
const configDb = require(__basedir + '/config/dbConfig');
...
class Tabela {
    ...
    async consultarPorId(id) {
        ...
        try {
            ...
            return modelConverter(rows, this._fields)[0];
        } catch (err) {
            throw Error(err);
        } finally {
            if (!conexaoOpen) {
    ...
    async consultar(criterio, orderBy, fieldsConsulta, groupTable = true) {
        let result = GeradorSql.sqlConsulta(this, fieldsConsulta, criterio, orderBy);
        ...
        try {
            ...
            let rows = await this._dao.executarSql(result.sql, result.params === undefined ? undefined : result.params);
            return modelConverter(rows, this._fields, groupTable);
        } catch (err) {
            throw Error(err);
        } finally {
            if (!conexaoOpen) {
    ...
    async consultaPaginada(first, skip, criterio, orderBy, fieldsConsulta, groupTable = true) {
        try {
            let totalReg = 0;
            let sqlDados = GeradorSql.sqlConsultaPaginada(this, fieldsConsulta, criterio, orderBy);
            let sqlTotal = GeradorSql.sqlConsultaPaginadaTotal(this, sqlDados.result.consulta);

            let valores = [];
            valores.push(first);
            valores.push(first * skip);

            if (sqlDados.result.params) {
                valores = valores.concat(sqlDados.params);
            }

            await this._dao.openConexao(false);
            let rows = await this._dao.executarSql(sqlDados.sql, valores);
            if (rows.length > 0) {
                let rowsT = await this._dao.executarSql(sqlTotal, sqlDados === undefined ? undefined : sqlDados.result.params);
                totalReg = parseInt(rowsT[0].COUNT, 0);
            }

            return { totalReg: totalReg, data: modelConverter(rows, this._fields, groupTable) };
        } catch (error) {
            throw Error(error);
        } finally {
            this._dao.closeConexao();
