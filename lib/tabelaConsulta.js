...
class TabelaConsulta {
  ...
  async consultar(criterio, orderBy, fieldsConsulta, groupTable = true) {
    try {
      let sqlDados = GeradorSql.sqlConsulta(this, fieldsConsulta, criterio, orderBy);
  ...
  async consultaPaginada(first, skip, criterio, orderBy, fieldsConsulta, groupTable = true) {
    try {
      ...
      if (rows.length > 0) {
        let rowsT = await this._dao.executarSql(sqlTotal, sqlDados.result.params === undefined ? undefined : sqlDados.result.params);
