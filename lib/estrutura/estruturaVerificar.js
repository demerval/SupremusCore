...
module.exports = {
  ...
  async verificarEstruturaModels(dirModels) {
    ...
    await executarVerificacao(new EstruturaVersaoModel());

    let UpdateVersaoModel = require('./models/updateVersaoModel');
    await executarVerificacao(new UpdateVersaoModel());
...
async function verificarCampo(fef, campo) {
  let sql = "SELECT RDB$RELATION_NAME, RDB$FIELD_NAME FROM RDB$RELATION_FIELDS "
    + "WHERE RDB$FIELD_NAME = '" + campo.name + "' AND "
...
async function ajustarCampo(fef, campo) {
  let ajustar = await verificarTamanhoCampo(fef.getNomeTabela(), campo.name, campo.maxLength);
  if (ajustar) {
